        ...
        .then(data => {
            ...
            if (req.session.loginId) {
                db.updateHistory(
                    req.session.loginId,
                    clean,
                    Number(results.documentSentimentScore)
                )
                    .then(() => {})
                    .catch(err =>
                        console.log(
                            "Error in db.updatehistory in POST /submitText: ",
                            err.message
                        )
                    );
            }
...
app.post("/getRephrasing", (req, res) => {
    rephraseSentence(req.body.text)
        ...
        .then(data => {
            let list = data.sentences.filter(
                sentence =>
                    Math.abs(sentence.sentenceSentiment) <
                    Math.abs(req.body.score)
            );
            res.json(list);
        })
        .catch(err =>
            console.log("Error in POST /getRephrasing: ", err.message)
        );
});
...
app.post("/register", (req, res) => {
    bc.getHash(req.body.password)
        ...
        .then(({ rows }) => {
            ...
            res.json({ loggedIn: true, userId: rows[0].id });
        })
        .catch(err => {
