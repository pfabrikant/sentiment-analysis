...
export function Main() {
    ...
    const valueOfEvaluation = useSelector(
        state => state && state.valueOfEvaluation
    );
    const highlightedText = useSelector(
        state => state && state.highlightedText
    );
    ...
    // function that creates the html for the highlights div (a div hidden behind the textarea)
    var composeHighlightedText = function() {
        ...
        positiveSentences.forEach(
            sentence =>
                (string = string.replace(
                    sentence,
                    `<mark class="positive">${sentence}</mark>`
                ))
        );
        negativeSentences.forEach(
            sentence =>
                (string = string.replace(
                    sentence,
                    `<mark class="negative">${sentence}</mark>`
                ))
        );
        dispatch(updateHighlightedText(string));
    };

    //calculating the general sentiment of the document by callibrating the sentiment
    // score with the magnitude score of the document
    let valOfEv;
    useEffect(() => {
        if (textEvaluation) {
            ...
            if (Math.abs(textEvaluation.documentSentimentScore) < 0.2) {
                if (
                    0 <
                    Math.abs(
                        textEvaluation.documentSentimentMagnitude -
                            Math.abs(textEvaluation.documentSentimentScore)
                    ) <
                    0.5
                ) {
                    valOfEv = "neutral";
    ...
    useEffect(() => {
        sentencesDetailedResultsRef.current &&
            sentencesDetailedResultsRef.current.scrollIntoView({
                behavior: "smooth",
                block: "end"
            });
    ...
    useEffect(() => {
        entitiesDetailedResultsRef.current &&
            entitiesDetailedResultsRef.current.scrollIntoView({
                behavior: "smooth",
                block: "end"
            });
                    ...
                    if (sentence.sentenceText.includes(sample)) {
                        instance
                            .post("/getRephrasing", {
                                ...
                                score: sentence.sentenceSentiment
                            })
                            .then(({ data }) => {
                                dispatch(
                                    updateRephrasing(sentence.sentenceId, data)
                                );
                            })
                            .catch(err =>
                                console.log(
    ...
    function insertRephrase(text) {
        let sentenceToReplace = textEvaluation.sentences.filter(
            sentence => sentence.sentenceId == rephrasingId
        )[0].sentenceText;
    ...
    return (
        <div className="center">
            <div className="container">
                ...
                ></textarea>
                {rephrasing && rephrasing.length > 0 && (
                    <div className="rephrasing">
                        ...
                        <h3>
                            The sentiment score of the chosen sentence is{" "}
                            {
                                textEvaluation.sentences[rephrasingId - 1]
                                    .sentenceSentiment
                            }
                        </h3>
                        <Slider
                        ...
                        <h4>
                            {" "}
                            A selection of rephrasings with a more neutral
                            sentiment score:
                        </h4>
            ...
            <div className="submit">
                <button onClick={() => submitText(inputText)}>
                    Submit text
                </button>
            </div>
            ...
            {textEvaluation && (
                <div className="resultsDiv">
                    <h2>
                        Your text appears to have a {valueOfEvaluation}{" "}
                        sentiment
                    </h2>
                    <div className="flex">
                        {" "}
                    ...
                    <div className="more">
                        <h4>
                            {" "}
                            Would you like a more detailed analysis of the
                            sentences and entities in your text?
                        </h4>
                        <div className="detailed-information">
                            <button
                                onClick={() =>
                                    setSentencesDetailedResults(true)
                                }
                            >
                                Text and Sentences
                            </button>
                            {textEvaluation.entitiesAnalysis.length > 0 && (
                                <button
                                    onClick={() => {
                                        setEntitiesDetailedResults(true);
                                        entitiesDetailedResultsRef.current &&
                                            entitiesDetailedResultsRef.current.scrollIntoView(
                                                { behavior: "smooth" }
                                            );
                    ...
                    {sentencesDetailedResults && (
                        <div className="details">
                            <h3>Overall Sentiment</h3>
                            <p>
                                {" "}
                                The overall sentiment score of the text is{" "}
                                {textEvaluation.documentSentimentScore} [range
                                between -1 (very negative) and 1 (very
                                positive)] while the overall magnitude
                                (emotional intensity) of your text is{" "}
                                {textEvaluation.documentSentimentMagnitude}{" "}
                                [relative to the document&apos;s length]
                            </p>
                            {textEvaluation.sentences.length > 0 && (
                                <React.Fragment>
                                    <h3>
                                        Sentences that have a particularly high
                                        or low sentiment score:
                                    </h3>
                                    {textEvaluation.sentences.map(sentence => {
                                        if (
                                            sentence.sentenceSentiment < -0.15
                                        ) {
                                            return (
                                                <p key={sentence.sentenceId}>
                                                    ...
                                                    and a magnitude score of{" "}
                                                    {sentence.sentenceMagnitude}{" "}
                                                </p>
                                            );
                                        } else if (
                                            sentence.sentenceSentiment > 0.15
                            ...
                            <div
                                className="ref"
                                ref={sentencesDetailedResultsRef}
                            ></div>
                        </div>
                    )}
                    {entitiesDetailedResults &&
                        textEvaluation.entitiesAnalysis.length > 0 && (
                        <div className="details">
                            <h3>
                                {" "}
                                    Entities in the text that have a
                                    particularly high or low sentiment score:
                            </h3>
                            {textEvaluation.entitiesAnalysis &&
                                    textEvaluation.entitiesAnalysis.map(
                                        entity => {
                                            if (
                                                entity.entitySentimentScore >
                                                0.1
                                            ) {
                                                return (
                                                    <p key={entity.entityName}>
                                                        The noun: &quot;
                                                        <mark className="positive">
                                                            {" "}
                                                            {
                                                        ...
                                                        and a magnitude score of{" "}
                                                        {
                                                            entity.entitySentimentMagnitude
                                                        }{" "}
                                                    </p>
                                                );
                                            }
                            ...
                            <div
                                className="ref"
                                ref={entitiesDetailedResultsRef}
                            ></div>
                        </div>
                    )}
                </div>
